...
Let's get started! You can grab my [JIT project](https://github.com/sophiajt/rustyjit), if you want to follow along. I've only tested this in OS X, but it should adaptable for other platforms.
...
_Note:_ You'll notice we don't explicitly call `libc::free()` after we allocate. For simplicity, we treat the JIT'ed functions we create as long-lived, but a full-blown JIT would likely use better memory hygiene.
